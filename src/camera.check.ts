...
for (const [from, to] of [[3.0, -3.0], [-3.0, 3.0], [0, 0.2], [1, -1]] as [number, number][]) {
  ...
  assert.ok(Math.abs(step) <= MAX / 60 + 1e-12, `swing ${from} -> ${to} breaks the cap`)
}
near(swing(1.2, 1.2, 1 / 60, RATE, MAX), 1.2, 'already astern')
...
// must not spin, and the circle it draws must be big enough to read as a turn
// rather than as a pirouette — 7.5 units a second over 0.9 rad/s is a circle
// 8.3 units in radius, a good deal wider than the world's biggest landmark.
let cam = Math.PI
let turned = 0
for (let i = 0; i < 600; i++) {
  const next = swing(cam, cam + Math.PI / 2, 1 / 60, RATE, MAX)
  turned += next - cam
  cam = next
}
near(turned / 10, MAX, 'a held sideways push turns the camera at exactly the cap')
assert.ok(7.5 / MAX > 8, 'the tightest sustained circle is under 8 units of radius')
