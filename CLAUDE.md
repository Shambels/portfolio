...
- Models are Blender, generated by a script in `tools/` and opened to sculpt.
  `tools/palm.py` is the one that is not a landmark: it exports a library of
  parts `Isle.tsx` instances, and it argues its own case at the top of the file
  the way `surfer.py` does.
